...
def prepare_features(data):
    ...
    city_encoded = city_map.get(city, 3)
...
def update_notion(page_id, data, score):
    ...
    # 添加预测得分 (数字类型)
    props["予測反響数"] = {"number": score}

    if data.get("rent"):
        props["賃料"] = {"rich_text": [{"text": {"content": f"{data['rent']:,}円"}}]}
    if data.get("area_sqm"):
        props["占有面積"] = {"rich_text": [{"text": {"content": f"{data['area_sqm']}㎡"}}]}
    if data.get("built_year"):
        props["築年数"] = {"rich_text": [{"text": {"content": f"{2026 - data['built_year']}年"}}]}
    if data.get("walk_minutes"):
        props["徒歩"] = {"rich_text": [{"text": {"content": f"{data['walk_minutes']}分"}}]}
    if data.get("floor_plan"):
        props["構造（RCほか）"] = {"rich_text": [{"text": {"content": data["floor_plan"]}}]}
...
def main():
    ...
    for page in pages:
        ...
        title = props.get("物件番号", {}).get("title", [])
        if title:
            bukken_no = title[0].get("text", {}).get("content", "")
            if bukken_no:
                bukken_list.append((bukken_no, page_id))
    ...
    try:
        ...
        for i, (bukken_no, page_id) in enumerate(bukken_list):
            ...
            try:
                ...
                # 所在地
                m = re.search(r'東京都[^区市]*([\u4e00-\u9fa5]+[区市])', text)
                if m:
                    data["city"] = m.group(1)
